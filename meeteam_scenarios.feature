...
Feature: 인증 및 환경 설정
  코드 수정 없이 설정 파일만으로 환경별 배포가 가능해야 한다

  Scenario: 로컬 환경 로그인
    Given 환경 설정이 "local"로 되어 있다
    When 사용자가 로그인한다
    Then 쿠키 도메인이 "localhost"로 설정된다
    And 로그인이 정상적으로 완료된다

  Scenario: 운영 환경 로그인
    Given 환경 설정이 "production"으로 되어 있다
    When 사용자가 로그인한다
    Then 쿠키 도메인이 ".meeteam.com"으로 설정된다
    And 로그인이 정상적으로 완료된다

  Scenario: Access Token 만료 후 갱신
    Given 사용자가 로그인된 상태이다
    And Access Token이 만료되었다
    When 사용자가 API를 요청한다
    Then Refresh Token으로 새로운 Access Token이 발급된다
    And API 요청이 정상적으로 처리된다

  Scenario: Refresh Token 만료 시 재로그인 요구
    Given 사용자가 로그인된 상태이다
    And Access Token과 Refresh Token 모두 만료되었다
    When 사용자가 API를 요청한다
    Then "다시 로그인해주세요" 메시지를 받는다
    And 로그인 페이지로 이동한다

  Scenario: Google 소셜 로그인
    Given 사용자가 Google 계정을 가지고 있다
    When Google 로그인 버튼을 클릭하고 인증을 완료한다
    Then 회원가입 또는 로그인이 완료된다
    And 메인 페이지로 이동한다

  Scenario: 소셜 로그인 시 이메일 미제공
    Given 사용자가 GitHub 계정을 가지고 있다
    And GitHub 계정에 이메일이 비공개로 설정되어 있다
    When GitHub 로그인 버튼을 클릭하고 인증을 완료한다
    Then 로그인이 거부된다
    And "이메일 정보가 필요합니다" 메시지를 받는다
