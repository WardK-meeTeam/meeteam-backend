...
기능: 프로젝트 관리
  ...
  시나리오: 리더가 지원자 목록을 확인할 수 있다
    ...
    그리고 각 지원자의 프로필 사진, 이름, 지원 필드, 지원 포지션, 지원일, 이메일, 지원서 내용을 확인할 수 있다.

  @applicant-management @happy-path
  시나리오: 리더가 지원서 상세 내용을 확인할 수 있다
    먼저 "박지민"이 다음 내용으로 지원한 상태이다:
      | 지원 포지션 | 나이  | 성별 | 자기소개                        |
      | 웹프론트엔드  | 25세 | 여성 | 리액트 경험 3년, 열정적입니다.   |
    그리고 "홍길동" 리더가 로그인한 상태이다
    만약 "홍길동"이 "박지민"의 지원서 상세를 조회하면
    그러면 지원 포지션 "웹프론트엔드"를 확인할 수 있다
    그리고 지원자의 프로필 사진, 이름, 나이, 성별, 지원 사유 및 자기소개를 확인할 수 있다
