...
Feature: 회원 인증
  ...
  시나리오: 잘못된 비밀번호로 로그인하면 실패한다
    만약 "홍길동"이 이메일 "hong@meeteam.com"과 비밀번호 "wrongpassword"로 로그인을 시도하면
    그러면 로그인에 실패한다
    그리고 "이메일 또는 비밀번호가 올바르지 않습니다" 메시지를 확인한다

  @login @error-case
  시나리오: 가입하지 않은 이메일로 로그인하면 실패한다
    만약 "미가입자"가 이메일 "unknown@meeteam.com"과 비밀번호 "anypassword"로 로그인을 시도하면
    그러면 로그인에 실패한다
    그리고 "이메일 또는 비밀번호가 올바르지 않습니다" 메시지를 확인한다
  ...
  @oauth @happy-path
  시나리오: Google 계정으로 처음 로그인하면 추가 정보 입력 페이지로 이동한다
    먼저 "새사용자"가 Google 계정으로 처음 로그인을 시도한다
    그러면 "추가 정보 입력" 페이지로 이동한다
    그리고 회원가입 전용 토큰을 발급받는다

  @oauth @happy-path
  시나리오: 기존 Google 계정 사용자는 바로 로그인된다
    먼저 "김영희"가 Google 계정으로 가입한 기존 회원이다
    만약 "김영희"가 Google 계정으로 로그인을 시도하면
    그러면 로그인에 성공한다
    그리고 "김영희"은 MeeTeam 서비스를 이용할 수 있다

  @oauth @happy-path
  시나리오: GitHub 계정으로 로그인할 수 있다
    먼저 "개발자"가 GitHub 계정으로 가입한 기존 회원이다
    만약 "개발자"가 GitHub 계정으로 로그인을 시도하면
    그러면 로그인에 성공한다
    그리고 "개발자"은 MeeTeam 서비스를 이용할 수 있다
