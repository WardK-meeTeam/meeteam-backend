...
기능: 회원 인증
  ...
  시나리오: 모든 필수 정보를 입력하면 회원가입에 성공한다
    ...
    만약 다음 정보로 회원가입을 요청하면:
      | 이메일            | 비밀번호      | 이름   | 생년월일     | 성별 |
      | hong@example.com | password123  | 홍길동 | 1995-03-15  | 남성 |
    그러면 회원가입에 성공한다
    그리고 "홍길동" 회원이 생성된다

  @signup @error-case
  시나리오: 이미 가입된 이메일로는 회원가입할 수 없다
    먼저 "hong@example.com" 이메일로 가입된 회원이 존재한다
    만약 동일한 이메일 "hong@example.com"으로 회원가입을 요청하면
    그러면 회원가입에 실패한다
    그리고 "이미 존재하는 이메일입니다" 메시지를 확인한다

  @signup @validation
  시나리오: 비밀번호가 8자 미만이면 회원가입할 수 없다
    만약 비밀번호 "pass123"으로 회원가입을 요청하면
    그러면 회원가입에 실패한다
    그리고 "비밀번호는 최소 8자 이상이어야 합니다" 메시지를 확인한다

  @signup @validation
  시나리오: 이메일 형식이 올바르지 않으면 회원가입할 수 없다
    만약 이메일 "invalid-email"로 회원가입을 요청하면
    그러면 회원가입에 실패한다
    그리고 "올바른 이메일 형식이 아닙니다" 메시지를 확인한다

  @signup @validation
  시나리오: 필수 항목을 입력하지 않으면 회원가입할 수 없다
    만약 이름을 입력하지 않고 회원가입을 요청하면
    그러면 회원가입에 실패한다
    그리고 "필수 항목을 모두 입력해주세요" 메시지를 확인한다

  # ============================================================================
  # 이메일 중복 확인
  # ============================================================================

  @email-check @happy-path
  시나리오: 사용 가능한 이메일인지 확인할 수 있다
    먼저 "new@example.com" 이메일은 가입되지 않은 상태이다
    만약 "new@example.com" 이메일 중복 확인을 요청하면
    그러면 "사용 가능한 이메일입니다" 메시지를 확인한다

  @email-check @error-case
  시나리오: 이미 사용 중인 이메일은 사용할 수 없다
    먼저 "hong@example.com" 이메일로 가입된 회원이 존재한다
  ...
  시나리오: 가입되지 않은 이메일로 로그인하면 실패한다
    ...
    만약 "unknown@example.com"과 "anypassword"로 로그인을 요청하면
    그러면 로그인에 실패한다
    그리고 "이메일 또는 비밀번호가 올바르지 않습니다" 메시지를 확인한다
  ...
  시나리오: 리프레시 토큰이 없으면 재발급받을 수 없다
    ...
    그러면 토큰 재발급에 실패한다
    그리고 "유효하지 않은 리프레시 토큰입니다" 메시지를 확인한다
