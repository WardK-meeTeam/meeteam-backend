...
기능: 회원 인증
  ...
  @oauth @happy-path
  시나리오: 신규 회원이 Google 소셜 계정으로 회원가입을 완료한다
    먼저 "새사용자"가 Google OAuth 인증에 성공하여 회원가입 코드를 발급받았다
  ...
  시나리오: 기존 Google 계정 사용자는 바로 로그인된다
    ...
    그리고 "김영희"가 Google OAuth 인증에 성공하여 로그인 코드를 발급받았다
    만약 "김영희"가 OAuth 토큰 교환을 요청하면
    그러면 로그인에 성공한다
  ...
  시나리오: GitHub 계정으로 로그인할 수 있다
    ...
    그리고 "개발자"가 GitHub OAuth 인증에 성공하여 로그인 코드를 발급받았다
    만약 "개발자"가 OAuth 토큰 교환을 요청하면
    그러면 로그인에 성공한다
