...
|------|---------------|------|
| 모집중 | O | 아래 제약사항 적용 |
| 모집중단 (리더 수동) | X | 수정 불가, 모집 재개 후 수정 가능 |
...
### 3. 대기 지원자가 있는 포지션 삭제 시 처리

**규칙**: 대기 중인 지원자가 있는 포지션 삭제 시, 해당 지원자들의 지원이 모두 자동 거절 처리됨

```
예시:
- 백엔드 - Kotlin/Spring (현재 0/2명, 대기 지원자 3명)
- 포지션 삭제 시도
- 프론트에서 확인 모달 표시: "해당 포지션의 지원자들의 지원이 모두 거절됩니다 정말 삭제하시겠습니까?"
- 확인 → 대기 지원자 3명 자동 거절 처리 + 포지션 삭제 완료
- 취소 → 삭제 취소
```
...
1. 리더가 포지션 삭제 시도
2. 대기 지원자 존재 여부 확인
3. 존재하면 → 프론트에서 확인 모달 표시
4. 리더가 확인 클릭 → 대기 지원자 전원 자동 거절 처리 + 포지션 삭제
5. 리더가 취소 클릭 → 삭제 취소
```
...
| 나의 분야 (리더 포지션) | O | - |
| 모집 분야 추가 | O | - |
| 모집 분야 삭제 | 조건부 | 승인 인원 0명 (대기 지원자 있으면 자동 거절 후 삭제) |
| 모집 인원 증가 | O | - |
...
| 상황 | 에러 메시지 |
|------|------------|
| 리더 수동 모집중단 상태에서 수정 시도 | "모집이 중단된 상태에서는 수정할 수 없습니다. 모집을 재개한 후 수정해주세요." |
| 현재 인원 이하로 축소 시도 | "현재 승인된 인원({n}명)보다 적게 설정할 수 없습니다." |
| 승인 인원이 있는 포지션 삭제 시도 | "승인된 팀원이 있는 포지션은 삭제할 수 없습니다." |
| 대기 지원자가 있는 포지션 삭제 확인 | (프론트 모달) "해당 포지션의 지원자들의 지원이 모두 거절됩니다 정말 삭제하시겠습니까?" |
...
|------|----------|
| 2026-02-21 | 최초 작성 |
| 2026-02-23 | 대기 지원자가 있는 포지션 삭제 정책 변경 (삭제 불가 → 확인 후 자동 거절 처리) |
| 2026-02-23 | 리더 수동 모집마감 → 모집중단으로 용어 변경 |